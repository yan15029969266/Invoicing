...
namespace Common
{
    public class Utils
    {
        ...
        public static string pictureRootPath = ConfigurationManager.AppSettings["imagePath"];
        ...
        public static string formartFilename(string name)
        {
            string extion = System.IO.Path.GetExtension(name);
            if (!string.IsNullOrEmpty(extion))
            {
                name = name.Substring(0, name.IndexOf(extion));
            }
            name = name.ToLower();
        ...
        public static string ReplaceCharacterEntities(string inputStr)
        {
            inputStr = inputStr.Replace("<br />", "\r\n");
            inputStr = inputStr.Replace("&#39;", "'");
            inputStr = inputStr.Replace("&rsquo;", "'");
            inputStr = inputStr.Replace("&lsquo;", "'");
            //inputStr = inputStr.Replace("&pound;", "£");
            inputStr = inputStr.Replace("&amp;", "&");
            inputStr = inputStr.Replace("&ndash;", "–");
            inputStr = inputStr.Replace("&nbsp;", " ");
            inputStr = inputStr.Replace("&bull;", "");
            inputStr = Regex.Replace(inputStr, @"&lt;[^&gt;]*&gt;", string.Empty, RegexOptions.IgnoreCase);
            return inputStr.Trim();
        }

        #region regex filename
        /// <summary>
        /// RegexName
        /// </summary>
        /// <param name="name"></param>
        ...
        #region fieldset Regex

        //public static Regex FieldRegex
        //{
        //    Regex[] reg = new Regex[10];
        public static Regex Regexcount = new Regex(@"<field:");
        public static Regex Regexitemname = new Regex(@"(?<=itemname=\"")\w[^\""]*");//提示文字
        public static Regex Regexfield = new Regex(@"(?<=<field:)\w*");//字段名
        public static Regex Regextype = new Regex(@"(?<=type=\"")\w[^\""]*");//数据类型
        public static Regex Regexautofield = new Regex(@"(?<=autofield=\"")\w[^\""]*");//表单类型
        public static Regex Regexisnull = new Regex(@"(?<=isnull=\"")\w[^\""]*");//不为空
        public static Regex Regexdefault = new Regex(@"(?<=default=\"")(\w|)[^\""]*");//默认值
        public static Regex Regexmaxlength = new Regex(@"(?<=maxlength=\"")\w[^\""]*");//最大
        public static Regex Regexinputtype = new Regex(@"(?<=inputtype=\"")\w[^\""]*");//inputtype
        public static Regex Regexvisible = new Regex(@"(?<=visible=\"")\w[^\""]*");//visible
        public static Regex Regexfieldstr = new Regex(@"<field:.*?>");//整个
        public static Regex Regexfk = new Regex(@"(?<=fk=\"")\w[^\""]*");//fk
        public static Regex Regexfkstr = new Regex(@"(?<=fkstr=\"")\w[^\""]*");//fkstr
        public static Regex Regexparameter = new Regex(@"(?<=parameter=\"")\w[^\""]*");//parameter
        public static Regex Regexfunction = new Regex(@"(?<=function=\"")\w[^\""]*");//function

        /// <summary>
        /// GetValueByRegexMatch
        /// </summary>
        /// <param name="regex"></param>
        /// <param name="inputstr"></param>
        /// <returns></returns>
        public static string GetValueByRegexMatch(Regex regex, string inputstr)
        {
            try
            {
                return regex.Match(inputstr).Value;
            }
            catch (Exception ex)
            {
                return "";
        ...
        #region 判断DataTable null or empty
        /// <summary>
        /// dt
        /// </summary>
        /// <param name="dt"></param>
        /// <returns></returns>
        public static bool DataTableIsNullOrEmpty(DataTable dt)
        {
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        public static bool ObjectIsNullOrEmpty(object drc)
        {
            if (drc != null && !string.IsNullOrEmpty(drc.ToString()))
            {
                return false;
            }
            else
        ...
        #region WriteJavaScript
        /// <summary>
        /// 向客户端发送Javascript脚本
        /// </summary>
        /// <param name="script"></param>
        public static void WriteJavaScript(string script, bool end)
        {
        ...
        #region GetTypeByDataType
        /// <summary>
        /// GetControlByDataType
        /// </summary>
        /// <param name="tablename"></param>
        /// <param name="colarr"></param>
        /// <returns></returns>
        //public static string GetControlByDataType(string item, string datatype, string id, string defaults, string maxlength, string inputtype, string isnull, string fk, string fkstr, string parameter, string function, string dirid)
        //{
        //    TCG.CMS.BLL.ChannelTypeBLL bll = new BLL.ChannelTypeBLL();
        //    string type = "";
        //    string validstr = "";
        //    if (isnull == "false")
        //    {
        //        validstr = "<span style=\"display:none;color:red;\" id=\"valid_addtion_" + id + "\">*required</span>";
        //    }
        //    if (fk == "y" && !string.IsNullOrEmpty(fkstr))
        //    {
        //        #region FK
        //        //if (fkstr.Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries).Length > 1)
        //        if (fkstr.Contains("tree-"))
        //        {
        //            TCG.CMS.BLL.DirBll bll_dir = new BLL.DirBll();
        //            string[] fkstrArr = fkstr.Split('^');
        //            string dirTreeStr = fkstrArr[0].Replace("tree-", "").Trim();
        //            string getType = fkstrArr.Length > 1 ? fkstrArr[1] : "";
        //            if (!string.IsNullOrEmpty(dirTreeStr))
        //            {
        //                List<TCG.CMS.Model.Directorys> dirList = bll_dir.GetDirectoryByIdStr(dirTreeStr);
        //                switch (inputtype.ToLower())
        //                {
        //                    case "select":
        //                        //type += "<select id=\"addtion_" + tablename + "_id\" name=\"addtion_" + tablename + "_id\" data-field=\"" + tablename + "_id\">";
        //                        foreach (TCG.CMS.Model.Directorys dir in dirList)
        //                        {
        //                            //type += "<td><input type=\"radio\" id=\"" + id + "_" + s.Split('|')[1] + "\" name=\"addtion_" + id + "\"  data-field=\"" + id + "\" value=\"" + s.Split('|')[1] + "\" " + ((s.Split('|')[1] == defaults) ? "checked='checked'" : "") + " />" + s.Split('|')[0] + @"</td>";
        //                        }
        //                        break;
        //                    case "radio":
        //                        type += "<table><tr>";
        //                        foreach (TCG.CMS.Model.Directorys dir in dirList)
        //                        {
        //                            type += "<td><input type=\"radio\" data-field=\"" + id + "\" id=\"addtion_" + id + "_" + dir.dirId + "\" name=\"addtion_" + id + "\" value=\"" + (getType == "i" ? dir.dirId.ToString() : dir.dirName) + "\" " + ((dir.dirName == defaults) ? "checked='checked'" : "") + " />" + dir.dirName + @"</td>";
        //                        }
        //                        type += "</tr></table>";
        //                        break;
        //                    case "checkbox":
        ...
        //                        foreach (TCG.CMS.Model.Directorys dir in dirList)
        //                        {
        //                            type += "<td><input type=\"checkbox\" data-field=\"" + id + "\" id=\"addtion_" + id + "_" + dir.dirId + "\" name=\"addtion_" + id + "\" value=\"" + (getType == "i" ? dir.dirId.ToString() : dir.dirName) + "\"  />" + dir.dirName + @"</td>";
        //                        }
        //                        type += "</tr></table>";
        //                        break;
        //                    case "checkboxlist":
        //                        type += "<div style=\"overflow: scroll; height: 120px; width: 300px; overflow-x: hidden; border: 1px solid #cccccc; padding: 2px 0 2px 5px; margin: 5px 0;\">";
        //                        foreach (TCG.CMS.Model.Directorys dir in dirList)
        //                        {
        //                            type += "<span style='display:block;'><input type=\"checkbox\" data-field=\"" + id + "\" id=\"addtion_" + id + "_" + dir.dirId + "\" name=\"addtion_" + id + "\" value=\"" + (getType == "i" ? dir.dirId.ToString() : dir.dirName) + "\" />" + dir.dirName + @"</span>";
        //                        }
        //                        type += "</div>";
        //                        break;
        //                }
        ...
        //        else
        //        {
        //            string[] fkArr = fkstr.Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
        //            string channeltypeid = fkArr[0];
        //            string tableField = fkArr[1];
        //            string customStr = "";
        //            string customFields = "";
        ...
        //            {
        //                customStr = fkArr[2];
        //                Regex cReg = new Regex(@"{\w+}");
        //                MatchCollection cmArr = cReg.Matches(customStr);
        //                foreach (Match m in cmArr)
        //                {
        //                    customFields += m.Value.Replace("{", "").Replace("}", "") + ",";
        //                    customFieldList.Add(m.Value.Replace("{", "").Replace("}", ""));
        //                }
        //                customFields = Utils.DelLastComma(customFields);
        //            }
        ...
        //            DataTable dt_input = bll.GetAddtionDataTableByChannelTypeId(int.Parse(channeltypeid), "id," + searchFields, tableField);
        //            switch (inputtype.ToLower())
        //            {
        //                case "select":
        //                    //type += "<select id=\"" + tablename + "_" + tableField + "\" name=\"" + tablename + "_" + tableField + "\">";
        //                    type += "<select id=\"addtion_" + id + "\" name=\"addtion_" + id + "\" data-field=\"" + id + "\">";
        //                    foreach (DataRow dr in dt_input.Rows)
        //                    {
        //                        string display = dr[tableField].ToString();
        //                        if (!string.IsNullOrEmpty(tableField))
        //                            display = customStr;
        //                        foreach (string cf in customFieldList)
        //                            display = new Regex(@"{\w+}").Replace(display, dr[cf].ToString(), 1);//Regex.Replace(customStr, @"{\w+}", dr[cf].ToString());
        //                        type += "<option value='" + dr["id"] + "' " + ((dr[tableField].ToString() == defaults) ? "selected='selected'" : "") + ">" + display + "</option>";
        //                    }
        //                    type += "</select>";
        //                    break;
        //                case "radio":
        ...
        //                    foreach (DataRow dr in dt_input.Rows)
        //                    {
        //                        type += "<td><input type=\"radio\" data-field=\"" + id + "\" id=\"addtion_" + id + "_" + dr["id"] + "\" name=\"addtion_" + id + "\" value=\"" + dr["id"] + "\" " + ((dr[tableField].ToString() == defaults) ? "checked='checked'" : "") + " />" + dr[tableField] + @"</td>";
        //                    }
        //                    type += "</tr></table>";
        //                    break;
        //                case "checkbox":
        ...
        //                    foreach (DataRow dr in dt_input.Rows)
        //                    {
        //                        type += "<td><input type=\"checkbox\" data-field=\"" + id + "\" id=\"addtion_" + id + "_" + dr["id"] + "\" name=\"addtion_" + id + "\" value=\"" + dr["id"] + "\" />" + dr[tableField] + @"</td>";
        //                    }
        //                    type += "</tr></table>";
        //                    break;
        //                case "checkboxlist":
        ...
        //                    foreach (DataRow dr in dt_input.Rows)
        //                    {
        //                        type += "<span style='display:block;'><input type=\"checkbox\" data-field=\"" + id + "\" id=\"addtion_" + id + "_" + dr["id"] + "\" name=\"addtion_" + id + "\" value=\"" + dr["id"] + "\" />" + dr[tableField] + @"</span>";
        //                    }
        //                    type += "</div>";
        //                    break;
        //                case "text":
        //                    type = "<input type=\"text\" data-field=\"" + id + "\" id=\"addtion_" + id + "\" name=\"addtion_" + id + "\" maxlength=\"" + maxlength + "\" />";
        //                    break;
        //            }

        //            //type += "&nbsp;&nbsp;&nbsp;<a href='javascript:void(0);' onclick=\"dialog = $.ligerDialog.open({ height: 600, title: '" + item + " Management', name: '" + item + "management', url: '../ChannelType/ManageList.aspx?dirid=0&channeltypeid=1021', width: 1024 });\">" + item + "</a>";
        //        }
        //        //else if (fkstr.Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries).Length == 1)
        //        //{
        //        //string channeltypeid = fkstr;
        //        //DataTable dt_columns = bll.GetDataTable(" id=" + channeltypeid + " and enable=1 ", "");
        //        //string tablename = dt_columns.Rows[0]["addtable"].ToString();
        //        //DataTable dt = new ArctypeDAL().GetList(" and channeltype=" + channeltypeid, "").Tables[0];

        //        //type += "<select id=\"addtion_" + tablename + "_id\" name=\"addtion_" + tablename + "_id\">";
        //        //foreach (DataRow dr in dt.Rows)
        //        //{
        //        //    type += "<option value='" + dr["id"] + "'>" + dr["typename"] + "</option>";
        //        //}
        //        //type += "</select>";
        //        //}
        //        #endregion
        ...
        //        {
        //            case "int":
        //                type = "<input type=\"text\" id=\"addtion_" + id + "\" style=\"width:200px;\" name=\"addtion_" + id + "\"  data-field=\"" + id + "\" onkeyup=\"value=value.replace(/[^\\d]/g,'') \"  onbeforepaste=\"clipboardData.setData('text',clipboardData.getData('text').replace(/[^\\d]/g,''))\" maxlength=\"" + maxlength + "\" " + (!string.IsNullOrEmpty(defaults) ? "value='" + defaults + "'" : "") + " />";
        //                break;
        //            case "float":
        //                type = "<input type=\"text\" id=\"addtion_" + id + "\" style=\"width:200px;\" name=\"addtion_" + id + "\"  data-field=\"" + id + "\" onkeyup=\"value=value.replace(/[^\\d+(\\.\\d+)?]/g,'')\" maxlength=\"" + maxlength + "\" " + (!string.IsNullOrEmpty(defaults) ? "value='" + defaults + "'" : "") + " />";
        //                break;
        //            case "text":
        //                if (inputtype == "textarea")
        //                {
        //                    type = "<textarea id=\"addtion_" + id + "\" name=\"addtion_" + id + "\"  data-field=\"" + id + "\" style=\"margin: 5px 0;\"  cols=\"80\" rows=\"8\"></textarea>";
        //                }
        //                else if (inputtype == "richtextarea")
        //                {
        //                    type = "<textarea id=\"addtion_" + id + "\" name=\"addtion_" + id + "\"  data-field=\"" + id + "\" ></textarea><script type=\"text/javascript\" >var ckeditor = CKEDITOR.replace('addtion_" + id + "',{ height: '180px', width: '505px' });</script>";
        //                    //type = "<textarea id=\"addtion_" + id + "\" name=\"addtion_" + id + "\"  data-field=\"" + id + "\" style=\"width:100%;height:300px;\" ></textarea><script type=\"text/javascript\" >UE.getEditor('addtion_" + id + "');</script>";
        //                }
        //                break;
        //            case "richtext":
        //                type = "<textarea id=\"addtion_" + id + "\" name=\"addtion_" + id + "\"  data-field=\"" + id + "\" ></textarea><script type=\"text/javascript\" >var ckeditor = CKEDITOR.replace('addtion_" + id + "');</script>";
        //                //type = "<textarea id=\"addtion_" + id + "\" name=\"addtion_" + id + "\"  data-field=\"" + id + "\" style=\"width:100%;height:300px;\" ></textarea><script type=\"text/javascript\" >UE.getEditor('addtion_" + id + "');</script>";
        //                break;
        //            case "enum":
        //                if (inputtype == "radio")
        ...
        //                        type += "<table><tr>";
        //                    }
        //                    foreach (string s in maxlength.Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries))
        //                    {
        //                        if (s.Contains("|"))
        //                        {
        //                            type += "<td><input type=\"radio\" id=\"" + id + "_" + s.Split('|')[1] + "\" name=\"addtion_" + id + "\"  data-field=\"" + id + "\" value=\"" + s.Split('|')[1] + "\" " + ((s.Split('|')[1] == defaults) ? "checked='checked'" : "") + " />" + s.Split('|')[0] + @"</td>";
        ...
        //                        else
        //                        {
        //                            type += "<td><input type=\"radio\" id=\"" + id + "_" + s + "\" name=\"addtion_" + id + "\"  data-field=\"" + id + "\" value=\"" + s + "\" " + ((s == defaults) ? "checked='checked'" : "") + " />" + s + @"</td>";
        //                        }
        //                    }
        //                    if (!string.IsNullOrEmpty(maxlength))
        //                    {
        ...
        //                        if (s.Contains("|"))
        //                        {
        //                            type += "<option value=\"" + s.Split('|')[1] + "\" " + ((s == defaults) ? "selected='selected'" : "") + ">" + s.Split('|')[0] + "</option>";
        //                        }
        //                        else
        //                        {
        //                            type += "<option value=\"" + s + "\" " + ((s == defaults) ? "selected='selected'" : "") + ">" + s + "</option>";
        //                        }
        //                    }
        //                    if (!string.IsNullOrEmpty(maxlength))
        //                    {
        ...
        //                break;
        //            case "datetime":
        //                type = "<input type=\"text\" class=\"Wdate\" id=\"addtion_" + id + "\" name=\"addtion_" + id + "\"  data-field=\"" + id + "\" style=\"width:200px;\" onclick=\"WdatePicker({ lang: 'en', skin: 'twoer',dateFmt:'yyyy-MM-dd'});\" />";
        //                break;
        //            case "varchar":
        //                if (inputtype == "text")
        //                {
        //                    type = "<input type=\"text\" id=\"addtion_" + id + "\" style=\"width:200px;\" name=\"addtion_" + id + "\" data-field=\"" + id + "\" maxlength=\"" + maxlength + "\" " + (!(string.IsNullOrEmpty(defaults) || defaults == "''") ? "value='" + defaults + "'" : "") + " />";
        //                }
        //                else if (inputtype == "textarea")
        //                {
        //                    //type = "<textarea id=\"addtion_" + id + "\" name=\"addtion_" + id + "\" data-field=\"" + id + "\" ></textarea><script type=\"text/javascript\" >var ckeditor = CKEDITOR.replace('addtion_" + id + "');</script>";
        //                    //type = "<textarea id=\"addtion_" + id + "\" name=\"addtion_" + id + "\" data-field=\"" + id + "\" style=\"width:100%;height:300px;\" ></textarea><script type=\"text/javascript\" >UE.getEditor('addtion_" + id + "');</script>";
        //                    type = "<textarea id=\"addtion_" + id + "\" name=\"addtion_" + id + "\"  data-field=\"" + id + "\" style=\"margin: 5px 0;\"  cols=\"80\" rows=\"5\"></textarea>";
        //                }
        //                break;
        //            case "set":
        //                if (!string.IsNullOrEmpty(maxlength))
        ...
        //                    if (s.Contains("|"))
        //                    {
        //                        type += "<td><input type=\"checkbox\" id=\"" + id + "_" + s.Split('|')[1] + "\" name=\"addtion_" + id + "\" data-field=\"" + id + "\" value=\"" + s.Split('|')[1] + "\" />" + s.Split('|')[0] + @"</td>";
        //                    }
        //                    else
        //                    {
        //                        type += "<td><input type=\"checkbox\" id=\"" + id + "_" + s + "\" name=\"addtion_" + id + "\" data-field=\"" + id + "\" value=\"" + s + "\" />" + s + @"</td>";
        //                    }
        //                }
        //                if (!string.IsNullOrEmpty(maxlength))
        //                {
        ...
        //                break;
        //            case "bool":
        //                type = "<table><tr><td><input type=\"radio\" id=\"" + id + "_yes\" name=\"addtion_" + id + "\" data-field=\"" + id + "\" checked='checked' value='1' />Yes</td><td><input type=\"radio\" id=\"" + id + "_no\" name=\"addtion_" + id + "\" data-field=\"" + id + "\" value='0' />No</td></tr></table>";
        //                break;
        //            case "image":
        //                //if (string.IsNullOrEmpty(parameter))
        //                parameter = "from=" + item + "&index=addtion_" + id + "&id=" + dirid + "&" + (!string.IsNullOrEmpty(parameter) ? parameter : "path=/resource/" + dirid + "/");
        //                type = "<div class=\"clearfix\" style=\"padding:5px 0;\"><img width='200' height='90' src=\"../images/nopic.jpg\" id=\"addtion_" + id + "\" style=\"float:left;\" /><input type=\"hidden\" name=\"addtion_" + id + "\" data-field=\"" + id + "\" /><input type=\"button\" value=\"Browse...\" onclick=\"var imgval=$(&quot;input[name='addtion_" + id + "']&quot;).val();window.open('../imageManage.htm#" + parameter.Replace('|', '&') + "&single=1&rid='+imgval+'&ran=" + new Random().NextDouble() + "', 'newwindow', 'height=500,width=1020,top=0,left=0,toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no');\" /><input type='button' value='delete' id='del_addtion_" + id + "' onclick=\"$('#addtion_" + id + "').attr('src', '../images/nopic.jpg'); $(&quot;input[name='addtion_" + id + "']&quot;).val('');\" />";
        //                //type += "<input id=\"btnUpload\" class=\"btn\" type=\"button\" value=\"Upload\" onclick=\" win= showEditWindow('/web/Resource/Upload3.aspx?" + parameter + "&single=1', 'Upload', '80%', '550', 'Upload');\"></div>";
        //                break;
        //            case "images":
        //                parameter = "from=" + item + "&index=addtion_" + id + "&id=" + dirid + "&" + (!string.IsNullOrEmpty(parameter) ? parameter : "path=/resource/" + dirid + "/");
        //                type = "<ul class=\"images_ul\" id=\"addtion_" + id + "\"></ul><input type=\"hidden\" name=\"addtion_" + id + "\" data-field=\"" + id + "\" /><input type=\"button\" value=\"Browse...\" onclick=\"var imgval=$(&quot;input[name='addtion_" + id + "']&quot;).val();window.open('../imageManage.htm#" + parameter + "&single=0&rid='+imgval+'&ran=" + new Random().NextDouble() + "', 'newwindow', 'height=500,width=1020,top=0,left=0,toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no');\" />";
        //                //type += "<input id=\"btnUpload\" class=\"btn\" type=\"button\" value=\"Upload\" onclick=\" win= showEditWindow('/web/Resource/Upload3.aspx?" + parameter + "&single=0', 'Upload', '80%', '550', 'Upload');\">";
        //                break;
        //            case "upload":
        //                parameter = "from=" + item + "&index=addtion_" + id + "&id=" + dirid + "&" + (!string.IsNullOrEmpty(parameter) ? parameter : "path=/resource/" + dirid + "/");
        //                type = "<div class=\"clearfix\" style=\"padding:5px 0;\"><input type=\"text\" style='width:200px;' name=\"addtion_" + id + "\" data-field=\"" + id + "\" /><input type=\"button\" value=\"Browse...\" onclick=\"var imgval=$(&quot;input[name='addtion_" + id + "']&quot;).val();window.open('../resourceManage.htm#" + parameter.Replace('|', '&') + "&single=1&rid='+imgval+'&ran=" + new Random().NextDouble() + "', 'newwindow', 'height=500,width=1020,top=0,left=0,toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no');\" /><input type='button' value='delete' id='del_addtion_" + id + "' onclick=\"$('#addtion_" + id + "').attr('src', '../images/nopic.jpg'); $(&quot;input[name='addtion_" + id + "']&quot;).val('');\" />";
        //                //type += "<input id=\"btnUpload\" class=\"btn\" type=\"button\" value=\"Upload\" onclick=\" win= showEditWindow('/web/Resource/Upload3.aspx?" + parameter + "&single=1', 'Upload', '80%', '550', 'Upload');\"></div>";
        //                break;
        //        }
        //        #endregion
        //    }
        //    if (type.Contains("type=\"text\"") || type.Contains("<select"))
        //    {
        //        type += validstr;
        //    }
        //    //if (!string.IsNullOrEmpty(function))
        //    //{
        //    //    type += "<script type=\"text/javascript\" >$(function(){" + function + ";});</script>";
        //    //}
        //    return type;
        //}
        /// <summary>
        ...
        /// <returns></returns>
        public static string[] GetTypeByDataType(string datatype)
        {
            string[] datatypearr = null;
            if (string.IsNullOrEmpty(datatype))
                return null;

            switch (datatype)
            {
                case "int": datatypearr = new string[] { "text" }; break;
                case "float": datatypearr = new string[] { "text" }; break;
                case "varchar": datatypearr = new string[] { "text", "textarea" }; break;
                case "text": datatypearr = new string[] { "textarea", "richtextarea" }; break;
                case "richtext": datatypearr = new string[] { "textarea" }; break;
                case "bool": datatypearr = new string[] { "radio" }; break;
                case "datetime": datatypearr = new string[] { "texttime" }; break;
                case "enum": datatypearr = new string[] { "radio", "select" }; break;
                case "set": datatypearr = new string[] { "checkbox" }; break;
                case "image": datatypearr = new string[] { "text" }; break;
                case "images": datatypearr = new string[] { "text" }; break;
                case "upload": datatypearr = new string[] { "text" }; break;
            }
        ...
        public static void BeginProgress()
        {
            ...
            string templateFileName = HttpContext.Current.Server.MapPath("ProgressBar.htm");
        ...
        public static void SetProgress(string line, int percent, int total)
        {
            string jsBlock = "<script>SetPorgressBar('" + line + "','" + percent.ToString() + "','" + total.ToString() + "'); </script>";
